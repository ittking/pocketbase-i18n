...
// i18n translations
const I18N_STORAGE_KEY = "pbLanguage";
...
let _i18nLang = window.localStorage.getItem(I18N_STORAGE_KEY) || "en";
let _i18nListeners = [];

function _i18nGetNested(obj, path) {
...
window.app.i18nSetLang = function(lang) {
    if (!i18nTranslations[lang]) lang = "en";
    if (_i18nLang !== lang) {
        _i18nLang = lang;
        window.localStorage.setItem(I18N_STORAGE_KEY, lang);
        _i18nListeners.forEach(fn => fn(lang));
    }
};
window.app.i18nT = function(key, params) {
    const dict = i18nTranslations[_i18nLang] || i18nTranslations["en"];
    const val = _i18nGetNested(dict, key);
    if (val === undefined) {
        console.warn(`[i18n] Missing: ${key}`);
        return key;
    }
    return val.replace(/\{(\w+)\}/g, (_, k) => params && params[k] !== undefined ? params[k] : `{${k}}`);
};
window.app.i18nOnChange = function(fn) {
    _i18nListeners.push(fn);
    return () => {
        _i18nListeners = _i18nListeners.filter(f => f !== fn);
    };
};
window.app.i18nGetAvailableLangs = function() {
    return [
